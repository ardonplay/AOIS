pub mod floating_operations {
    ...
    pub fn decimal_to_float(mut decimal: f32) -> Float {
        let mut output = Float { mantissa: String::new(), index: String::new(), sign: 0 };
    ...
    pub fn basing(mut first: Float, second: Float, first_index: i32) -> Float {
        ...
        digit_mantissa *= i32::pow(10, (to_decimal(first.index.clone(), 8) - first_index) as u32);
        first.mantissa = to_binary(digit_mantissa, 23);
        first
    }

    pub fn binary_float_sum(mut first: Float, mut second: Float) {
        let mut res = Float { mantissa: String::new(), index: String::new(), sign: 0 };
        ...
        match first_index.cmp(&second_index) {
            Ordering::Less => {
                first = basing(first.clone(), second.clone(), first_index)
            }
            Ordering::Equal => {}
            Ordering::Greater => {
                second = basing(second.clone(), first.clone(), second_index)
            }
        }
        ...
        res.index = binary_sum(first.index, second.index);
        res.mantissa = binary_sum(first.mantissa, second.mantissa);
        let rezulator: f32 = to_decimal(res.clone().mantissa, 23) as f32 * f32::powf(10.0, (-to_decimal(res.clone().index, 8) / 2) as f32);
        println!("{}", rezulator);
